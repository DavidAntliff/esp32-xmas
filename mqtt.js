...
function color(mean, variance) {
	return Math.min(Math.max(0.0, round(mean + variance * randn_bm(), 1)), 1.0);
}

function publishTemps() {
	mqtt.publish("xmas/brightness", 15);
	mqtt.publish("xmas/red",   color(0.5, 0.3));
	mqtt.publish("xmas/green", color(0.5, 0.3));
	mqtt.publish("xmas/blue",  color(0.5, 0.3));
}
...
function publishLoop(delay_ms, publish_fn)
{
	...
	while (true)
	{
		...
		for (count = 0; count < Math.ceil(delay_ms / 1000.0); count++) {
			// Sleep for 0.01 second and handle a stop request
			try
			{
				Thread.sleep(1000);
			}
